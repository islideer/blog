...
excerpt: 分析深拷贝与浅拷贝的异同，并使用 JS 进行代码实现。
---

## 基本数据类型 与 引用数据类型

我们知道，在 JS 语言中，有**基本数据类型**和**引用数据类型**之分。
...
原因在于，在我们通过赋值运算符 `=` 对引用数据类型的变量进行赋值的时候，由于 JS 语言的特性，实际赋值的是 `obj` 这个变量的地址。（即 `{ a: 123, b: 456}` 这个对象存放在堆内存中的地址）
...
可以直观的看到，对于 `Set`、`Map`、`Error`、`ExpReg`、`Date` 等内置的 JS 对象而言，这个函数并没有按照我们预期的方式进行工作，它会将这些内置的 JS 对象当作普通对象来处理，我们再来改进一下这个函数，使他支持 JS 内置对象的克隆。
...
可以看到，第二版代码的处理更加科学和准确，它采用 `Object.prototype.toString` 来判断类型，并对内置的 JS 对象进行了处理，使它能更加精准的拷贝对象的属性。
