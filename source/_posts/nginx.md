...
> [Windows下nginx安装、配置与使用 - 脚本之家](https://www.jb51.net/article/86839.htm)
...
* 分类
  ...
  + **正向代理**
    ...
    （差不多意思就是，当你通过正向代理访问 Google 并进行搜索时， Google 的服务器只清楚这个请求来自哪台代理服务器（或ip地址），却不知道是来自哪个客户端。因为**客户端是通过代理服务器这个媒介来访访问网站的**。而不用代理直接打开百度并搜索时，百度是知道这个请求是具体来自哪个客户端（或说ip地址）的）
...
* [nginx配置入门 - nginx中文网](http://www.nginx.cn/591.html) 
