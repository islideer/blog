...
因为家园的todolist作业涉及到要自己部署，所以要学会适用于服务端的 `Linux` 发行版 `CentOS` 的基本操作，并完成部署作业

我下载了官网的CentOS7版本的镜像, 并安装在VM虚拟机中进行学习

> 安装教程参考了：[VM安装Centos7-菜鸟教程](https:#www.runoob.com/w3cnote/vmware-install-centos7.html)
...
> Linux内置了Vim编辑器，详细用法参考： [Vi/Vim 操作-菜鸟教程](https:#www.runoob.com/linux/linux-vim.html)
...
此命令创建了一个用户sam，其中 `-d` 和 `-m` 选项用来为登录名sam产生一个主目录`/home/sam` （ `/home` 为默认的用户主目录所在的父目录，所有用户的目录都应该在此目录下）。`-m` 命令保证了在目录不存在时自动创建目录
...
此命令将用户sam的登录 `Shell` 修改为 `ksh` ，主目录改为 `/home/z` ，用户组改为 `developer` 。
...
* 实际安装时建议选择语言为English，若选择其他语言很有可能发生意想不到的编码问题
